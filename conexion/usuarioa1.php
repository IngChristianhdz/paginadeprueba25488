...
if (mysqli_num_rows($result) > 0) {
    ...
    echo "Correo ya registrado";
            ...
            $result =  mysqli_query($usuario,"SELECT * FROM usuario  where cuenta = '$cuenta'");
    if (mysqli_num_rows($result) > 0) {

        $sql = "UPDATE  usuario set correo='$correo' where cuenta='$cuenta'";
        mysqli_query($usuario, $sql);
        
        $sql = "INSERT INTO registro (Correo, Contrasena, Sexo, Nombre, Domicilio, Ciudad, Estado, Telefono, Pregunta, Respuesta, Terminos) VALUES ('$correo', '$contra', '$sexo', '$nombre', '$domicilio', '$ciudad', '$estado', '$telefono', '$pregunta', '$respuesta', '$terminos')";
        mysqli_query($usuario, $sql);
        
        echo "Correo se Registro Satisfactoriamente";
     }else {            echo "cuenta no existe";     } 
